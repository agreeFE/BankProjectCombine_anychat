...
export class HcdWaveView extends Component {
    ...
    return <View style={{ backgroundColor: 'rgba(0,0,0,0.0)' }}>
      <ART.Surface width={this.surfaceWidth} height={this.surfaceHeigth} >
        ...
        <ART.Text strokeWidth={1} 
          ...
          fill="#FFFFFF"
          font={`12px "Helvetica Neue", "Helvetica", Arial`}>识别进度</ART.Text> */}
      </ART.Surface>
    </View>
  }
  ...
  artDrawWave() {
    ...
    if (powerPercent < 107) {
      ...
      for (var x = startX; x <= endX; x++) {
        var y = a * Math.sin(x / 180 * Math.PI*1.3 + 4 * b / Math.PI) * amplitude + currentLinePointY
        if (y < centerY) {
      ...
      linePath.close()
      let colors = [ "#0F77DC", "#35BDEF", ];
      let linearGradient = new ART.LinearGradient(colors, 0, 120, 90, 280);
      return (
  ...
  render() {
    return (
      <View style={{ width: this.props.surfaceWidth, height: this.props.surfaceHeigth, justifyContent: 'center', alignItems: 'center',  backgroundColor: '#000', }}>
        {this.typeView()}
      </View>
    )
  }
