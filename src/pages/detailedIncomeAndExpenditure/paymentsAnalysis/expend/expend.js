...
class Payments extends Component {
  constructor(props) {
    ...
    let expendData = [
      { item: "差旅", expendMoney: "5546" },
      { item: "房租", expendMoney: "1400" },
      { item: "餐饮", expendMoney: "2357" },
      { item: "购物", expendMoney: "3750" },
      { item: "保险", expendMoney: "1595" },
      { item: "其余", expendMoney: "1300" },
    ];
    // 计算支出的总金额
    ...
    this.state = {
      ...
      handledClickCircleIndex: 0, //被点击的圆环索引
      ...
      <>
        <View style={{ justifyContent: 'center', alignItems: 'center' }}>
          <View style={styles.title}>
            ...
            <View>
              <Text style={styles.titleItem}>￥ {formatMoney(this.state.totalExpend)}</Text>
            </View>
          </View>
          <View>
            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
              ...
              <View>
                ...
                <View style={{ position: 'relative', alignItems: 'center' }}>
                  ...
                  {/* 扇形图背景图 */}
                  <Image style={{ position: 'absolute', width: 245, height: 245, zIndex: -1 }} source={PAYMENTS_SECTORBG}></Image>
                  {/* 支出详情展示区 */}
                  <LinearGradient style={[styles.capitalDisplay]} colors={['#fff', '#f6f6f6']}>
                    <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                      <Text style={styles.expendType}>{this.state.expendType}</Text>
                      <Text style={styles.expendMoney}>￥{formatMoney(this.state.expendMoney)}</Text>
                    </View>
                  </LinearGradient>
                  {/* <BoxShadow setting={shadowOpt}> */}
                  <Text style={styles.viewDetails}>点击下方按钮，可刷新扇形图</Text>
                  {/* </BoxShadow> */}
                </View>
              </View>
              <View style={styles.expendClassification}>
                {/* 支出分类区 */}
                {
                  this.state.expendData.map((ele, index) => {
                    ...
                    return (
                      <TouchableWithoutFeedback key={ele.item} onPress={() => { this.setIndex(index,this.state.handledClickCircleIndex) }}>
                        <View style={[styles.expendCircle, styles.textCenter]}>
                          <Range ref={(view) => this.range.push(view)} curIndex={index} curClickIndex={this.state.handledClickCircleIndex}>
                            <Text>{ele.item}</Text>
                            <Text>{percent}%</Text>
                          </Range>
                        </View>
