...
class MineBankCard extends Component {
  ...
  himBankBox = ({item, index}) => {
    return (
      <TouchableWithoutFeedback onPress={() => {router.load('bankCardManager', {item,isOwn: false})}}>
  ...
  ownBankBox = ({item, index}) => {
    ...
    return (
      <TouchableWithoutFeedback onPress={() => {router.load('bankCardManager', {item,isOwn: true})}}>
      <View style={{ marginVertical: 10 }}>
