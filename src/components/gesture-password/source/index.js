...
export default class GesturePassword extends Component {
    constructor(props) {
        ...
        // alert(this.props.Width)
        this.Width = this.props.Width;
        this.Height = this.props.Height;
        this.isVertical = this.Height > this.Width;
        this.Top = this.isVertical ? (this.Height - this.Width)/1.5 * 1.25 : 10;
        this.Radius = this.isVertical ? this.Width / 10 : this.Width / 25;
        ...
        this.styles = {
            ...
            board: {
                ...
                position: 'absolute',
                left: this.isVertical ? 0 : this.Width/3.4,
                top: this.isVertical ? this.Top/1.5 : 30,
                width: this.Width,
                height: this.Height
            },
            msgText: {
        ...
        for (let i=0; i < 9; i++) {
            ...
            circles.push({
                isActive: false,
                x: p * (this.Radius * 2 + Margin) + Margin + this.Radius,
                y: q * (this.Radius * 2 + Margin) + Margin + this.Radius
            });
        }
    ...
    render() {
        ...
        return (
            <View style={[this.styles.frame, this.props.style, {flex: 1},{backgroundColor:this.props.backgroundColor}]}>
                {/*文字描述, 暂时删掉 todo */}
    ...
    onStart(e, g) {
        ...
        let x = this.isVertical ? e.nativeEvent.pageX : e.nativeEvent.pageX - this.Width/3.4;
        let y = this.isVertical ? e.nativeEvent.pageY - this.Top/1.25 : e.nativeEvent.pageY - 30;
    ...
    onEnd(e, g) {
        if ( this.isMoving ) {
            ...
            //清空轨道数据
            if(this.props.isClearPass == true){
                this.resetActive()
            }
        }
    }
