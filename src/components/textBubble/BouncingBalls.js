...
import { View, Image, StyleSheet, Dimensions, TouchableWithoutFeedback, TouchableOpacity, ImageBackground, Animated, Easing, Text } from 'react-native';
...
class BouncingBalls extends PureComponent {
  ...
  constructor(props) {
    ...
    this.screenWidth = Dimensions.get('window').width / 3.5;
  ...
  generateCircles() {
    ...
    for (var i = 0; i < amount; i++) {
      ...
      direction = Math.round(Math.random()) <= 0.5 ? -1 : 1;
      ...
      restStyles = {
        ...
        speedX: direction * this.getRangeFromMinToMax(minSpeed, maxSpeed) * 0.2,
        speedY: direction * this.getRangeFromMinToMax(minSpeed, maxSpeed) * 1.3,
      };
      ...
      item = imageBall ?
        ...
        <ImageBackground
          source={imageBall}
          style={[styles.circle, { ...innerStyle }, { ...style }]} {...restStyles}
        >
          <View>
            {this.props.children}
          </View>
